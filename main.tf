...
#   Name = "terraform-devEC2-01"
    Name = "terraform-devEC2-${count.index}"
  }
...
#   command = "sleep 120; ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ec2-user --private-key ~/Downloads/lel_aws.pem -i '${aws_instance.devEC2-01.public_ip},' deploy_webserver.yml"
    command = "sleep 120; ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ec2-user --private-key ~/Downloads/lel_aws.pem -i '${aws_instance.devEC2-"${count.index}".public_ip},' deploy_webserver.yml"
  }
}
...
# network_interface_id = "${aws_instance.devEC2-01.primary_network_interface_id}"
  network_interface_id = "${aws_instance.devEC2-${count.index}.primary_network_interface_id}"
}
